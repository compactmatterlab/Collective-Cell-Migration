...
filename = strcat('MCM_',date);
% rng(datenum(date));%......................................For repeatability
tic

% Model Definitions
runTime = 12/24*24*3600;%.........................Total runtime for model (sec)
...
Fmax = 10e-9;%........................Max force provided by single cell (N) 10e-9
...
Lp_cos = zeros(dataPnts,samples,nCells,1);
...
cellX =  zeros(dataPnts,samples,nCells,1); %....Rebranded cellDist in stats
...
parfor par=1:dataPnts
    ...
    p = [CGel(par),PFiber(par),Tsearch(par),FiberAI(par),rgdPeptides(par)];
...
%             SAcell(cell) = saellipsoid(cellAx(cell,:));
        ...
        %                 elseif cellOne == 1%bonds(cellOne) >= Bmax
        %                     dirAng(cellOne,cellTwo) = atan2d(norm...
        %                         (cross(cellDist(cellOne,:,cellTwo),...
        ...
        % %                     gpd(pos,:,cellOne) = fiberDir(cellOne,:);
        %                     
        %                 elseif cellOne == 2 || cellOne == 3
        %                     if norm(tFNet(cellOne,:)) > 5e-9
        %                         dirAng(cellOne,cellTwo) = atan2d(norm...
        %                             (cross(cellDist(cellOne,:,cellTwo),...
        ...
        %                         :,cellTwo),fiberDir(cellOne,:)));
        %                     gpd(pos,:,cellOne) = fiberDir(cellOne,:);
        %                 elseif cell == cellOne%bonds(cellOne) >= Bmax
        %                     dirAng(cellOne,cellTwo) = atan2d(norm...
        %                         (cross(cellDist(cellOne,:,cellTwo),...
        %                         pseudoVect(cellOne,:))),dot(cellDist(cellOne,...
        %                         :,cellTwo),pseudoVect(cellOne,:)));
        ...
        % %                     gpd(pos,:,cellOne) = fiberDir(cellOne,:);
        %                     
        %                 elseif norm(tFNet(cellOne,:)) > 5e-9
        %                     dirAng(cellOne,cellTwo) = atan2d(norm...
        %                         (cross(cellDist(cellOne,:,cellTwo),...
        %                         tFNet(cellOne,:))),dot(cellDist(cellOne,...
        %                         :,cellTwo),tFNet(cellOne,:)));
...
%                     vLpseudo(cell,:) = pseudoVect(cell,:);
        ...
        %             if pseudoVect(cell,:) ~= gPV(pos,:,cell)
        %                 fprintf('yeah not equal')
        %             end
...
%                         vLpseudo(cell,:) = pseudoVect(cell,:);%Might use for mvmt
        ...
        %                 end
                ...
                %=================================================================%
%                 pVTest(cell,:) = pseudoVect(cell,:);

               
                for cellOne = 1:nCells %Need to make accessible outside of parfor loop
                    %Commented out 8/29/18
% %                     if pos == 1
% %                         gpd(pos,:,cellOne) = fiberDir(cellOne,:);
% %                     elseif cellOne == 1
% %                         gpd(pos,:,cellOne) = pseudoVect(cellOne,:);
...
% %                     end
        ...
        %                     end
        %                 elseif cellOne > 1
        %                     di(cellOne,cellTwo) = rCell(cellOne);
        %                 end
        % %                 di(cellOne,cellTwo) = rCell(cellOne);
                    end
                end
...
%                 TTS(par,samp,i) = 1-TTC(par,samp,i);

        %         TTS(par,samp,i) = runTime-timeCol(i)*2;
                ...
                if runAvgVelocity == 1
                    ...
                    for q=1:(size(rw,1))/10:size(rw,1)
                        ...
                        dst(w,1,i) = sum(Mag(:,1));%...............................Gets distance traveled at each time point
                        w = w+1;
                    end
...
%                     R1(par,samp,:) = R1Tmp
                ...
                %Commented out 8/29/18 - Use is not clear, and indexing
                %needs updating
% %                 MSD(:,i,samp) = msd;
...
save(filename,'avgVel','cellX','LpR','D','alp','R1','R2','R3','outg','cont','retr','foll','p1','p2','TTC','TTS')
...
% save(filename1)
fprintf('Complete! Fuck Yeah....  End Time:  %s \n',datestr(now))
