...
alpha = 1e-9;%...............................Adhesive strength constant (N) 1e-8
Fcomp = 30e-9;%........................Compressive force during contact (N) 30e-9
...
avgSearchTime = 4;%...............................Pseudopod extension time
...
%             SE(cell) = dmin(cell)*etaSurf + etaCC*tsurf(cell);%Palsson version of SE
        ...
        for cellTwo = 1:nCells
            ...
            elseif ecks(cell,cellTwo) < 0 
                nFp = -1*Fcomp*otX(cell,cellTwo)*(-1*ecks(cell,cellTwo))^(3/2);
                Fp(cell,:,cellTwo) = nFp*nuCellDist(cell,:,cellTwo);
                
            % Adhesive force
            else
                nFp = alpha*otX(cell,cellTwo)*((ecks(cell,cellTwo)+...
...
%                     end
        ...
        for cell = 1:nCells
           if ~contracting(cell)
               if nnz(cellContact(cell,:)) > 0
                   dMove(cell) = min(dMax(cell),dMove(cell));
               end
           end
        end
        for cell = 1:nCells
