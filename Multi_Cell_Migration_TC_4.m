...
% krCell = k_cell/(k_cell + k_ecm);
% k_ecm = ones(num_cells,1)*k_ecm;
...
%             rCell(cell) = a(cell)/(0.14); %
            ...
            for j=1:localFibers(cell)
                angle1(j,1) = acosd(dot(fiber(j,:),ve)/(norm(fiber(j,:))*norm(ve)));   %Fiber angles with respect to cell's previous direction
            end
...
%             %Finds the angle from cell-cell vector to fiber direction vector
%             for k = 1:cell % Not gonna work as intended
%                 if k == cell
%                     continue
%                 end
%                 if fiberDir(cell,:) == 0
%                     pseudoCellAng(cell,k) = 0;
%                     continue
%                 end
%                 pseudoCellAng(cell,k) = atan2d(norm(cross...
%                     (cellDist(cell,:,k),fiberDir(cell,:))),...
%                     dot(cellDist(cell,:,k),fiberDir(cell,:)));
%             end
            ...
            cross_L = L(min_idx);                                      %Distance to next crosslink
            crs_length(cell) = cross_L;
            numElmts(cell) = floor(cross_L/elmtSize);                %Number of elements to search along fiber
...
%                         *exp(-(kT_eff)/(bondsTotal*kB*T));%Bond friction
        ...
        for cellOne = 1:nCells 
            for cellTwo = 1:nCells
                ...
                di(cellOne,cellTwo) = sqrt(b(cellOne)^2*cos(pseudoCellAng(cellOne,cellTwo))^2 ...
                    + a(cellOne)^2*sin(pseudoCellAng(cellOne,cellTwo))^2);
            end
        end
...
%         for cellOne = 1:num_cells 
%             for cellTwo = 1:num_cells
%                 if cellOne == cellTwo
%                     continue
%                 end
%                 if surf(cellOne,cellTwo) == 0
%                     surf(cellOne,cellTwo) = 1e-15;
%                 end
%             end
%         end
...
%                 surf(cell,:) = surf(cell,:)./nSurf(cell);
%             end
...
%                 /((fb + fv)*k_ecm(cell)*pseudo_mag(cell));                                  %Velocity calculated by force generated
        ...
        if pseudoVect(cell,:) == 0
%         if F(cell) == 0
            FNet(cell,:) = -1*Fp;
        else
            FNet(cell,:) = -1*(F(cell)*((pseudoVect(cell,:)/norm(pseudoVect(cell,:))))) - Fp;
        end
    end
    %=====================================================================%
...
%                 Vz(cell) = 0;
    ...
    %         cVel(cell,:) = [Vx(cell),Vy(cell),Vz(cell)]; %Cell velocity vector 
            for v = 1:nCells
                if cVel(v,:) == 0
                    nuVel(v,:) = [0,0,0];
                else
...
%     plot3(random_walk(:,1,i),random_walk(:,2,i),random_walk(:,3,i),'Color','b')
    ...
    if i > 2
        plot3(random_walk(:,1,i),random_walk(:,2,i),random_walk(:,3,i),'Color','y')
    end
...
%     plot3(cont_pos(:,1,i),cont_pos(:,2,i),cont_pos(:,3,i),'Color','r')
    scatter3(random_walk(end,1,i),random_walk(end,2,i),random_walk(end,3,i),'MarkerEdgeColor','k','MarkerFaceColor','r')
end 
