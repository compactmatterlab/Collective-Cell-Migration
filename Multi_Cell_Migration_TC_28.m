...
% Model Definitions
runTime = 0.5*0.5*24*3600;%.........................Total runtime for model (sec)
...
fprintf('Running %d cell/s over %2.1f day/s. Start Time:  %s \n',nCells,...
    runTime/(24*3600),datestr(now))

%Code Sectional Toggles 
animation = 1;..............(1) Creates 3D Animation or (0) Creates 3D plot
...
contactRange = Dcell + Dcell;%........Contact range from cell center (um)
% contactRange = 0;
...
Fadhv = 600e-10;%...............................Adhesive strength constant (N) 1e-8
...
angAvoid = zeros(nCells,nCells);
nFNet = zeros(nCells,1);
sumSurfA = zeros(nCells,1);
FNET = zeros(1,3,nCells);
vDMove = zeros(nCells,3);
...
% Parameters
CGel = 3.7;%........................................Gel concentration (mg/ml)
...
%             a(cell) = 7.51;%Major axis of cell
            ...
            rCell(:) = 7.5;%.........Radius in Palsson Eqns
...
%             gVeAngle(pos,cell) = veAngle(cell);
%         elseif ve(cell,:) == gVe(pos-1,:,cell)
%             gVe(pos,:,cell) = zeros(3,1);
% %             gVeAngle(pos,cell) = zeros(1);
%         else
%             gVe(pos,:,cell) = ve(cell,:);
% %             gVeAngle(pos,cell) = veAngle(cell);
%         end
...
%--------------------------------------------------------------------------
    for cell = 1:nCells
        ...
        %contracting
        for cellOne = 1:nCells
            if bonds(cellOne) >= Bmin % TEMP FOR ADHES TEST; CHANGE TO BMAX 
                for cellTwo = 1:nCells
...
%                         following(cellTwo) = 1;
        ...
        for cellOne = 1:nCells
            for cellTwo = 1:nCells
                ...
                angAvoid(cellOne,cellTwo) = atand(1.5*rCell(cell)/nCellDist(cellOne,cellTwo));
                if pseudoCellAng(cellOne,cellTwo) < angAvoid(cellOne,cellTwo) &&...
                    cellContact(cellOne,cellTwo) == 1
%                     if contracting(cellOne)
%                     if bonds(cellOne) < Bmin
%                         newAngle(cellOne) = 1;
%                         bonds(cellOne) = 0;
%                     else
                        if rand > 0.5
                        %reverse polarity
                            fiberDir(cellOne,:) = -1*fiberDir(cellOne,:); 
                        else
                            newAngle(cellOne) = 1;
                        end
%                     end
...
%             if nnz(cellContact(cell,:)) == 0 && nnz(contracting(:)) == 0
                %Resets values
                Lpseudo(cell) = 0;
                time(cell) = 0;
                bonds(cell) = 0;
...
%             end
        ...
        elseif bonds(cell) >= Bmin && bonds(cell) < Bmax
            ...
            vLpseudo(cell,:) = pseudoVect(cell,:);
%             pV2(cell,:) = pseudoVect(cell,:);
        ...
        elseif bonds(cell) >= Bmax
            ...
            if not(contracting(cell))
                ...
                vLpseudo(cell,:) = pseudoVect(cell,:);%Might use for mvmt
%                 pV2(cell,:) = pseudoVect(cell,:);
            end
        end
        %=================================================================%
        ...
        for cellOne = 1:nCells 
            for cellTwo = 1:nCells
                ...
                di(cellOne,cellTwo) = a(cellOne)*b(cellOne)/(sqrt((b(cellOne)*cosd(...
                    pseudoCellAng(cellOne,cellTwo)))^2 ...
                    +(a(cellOne)*sind(pseudoCellAng(cellOne,cellTwo)))^2));
            end
        end
        ...
        for cellOne = 1:nCells 
            for cellTwo = 1:nCells
                ...
                elseif cellContact(cellOne,cellTwo)
                    ...
                    surfA(cellOne,cellTwo) = surfA(cellOne,cellTwo);%/...
...
%         SE(cell) = eay*etaSurf + etaCC*tsurf(cell);%Palsson version of SE
        ...
        for cellTwo = 1:nCells
            ...
            % Adhesive force
                ...
                Fp(cell,:,cellTwo) = nFp*nuCellDist(cell,:,cellTwo);
                if nFp < 0
                    fprintf('nFp is NEG and shouldnt be for ahesive')
                end
            end
...
%                 Lpseudo(cell) = Lpseudo(cell)-dot(fiberDir(cell,:),FNet(cell,:))/norm(fiberDir(cell,:));
%                 if Lpseudo(cell) == 0 && norm(FNET(pos-1,:,cell)) > 0 
%                     fprintf('Something something')
%                 end
                Lpseudo(cell) = Lpseudo(cell)-dMove(cell);
                if Lpseudo(cell) == 0 && norm(FNET(pos,:,cell)) > 0 
%                     fprintf('Something something')
                end
...
%                 cellPos(pos+1,:,cell) = cVel(cell,:)*dt+cellPos(pos,:,cell);
            else
        ...
        for cell = 1:nCells
            if contracting(cell)
                pMove = dot(pseudoVect(cell,:),vDMove(cell,:))/norm(pseudoVect(cell,:));
                ...
                if pMove > 0
                    pMove = min([pMove Lpseudo(cell)]);
                end
                Lpseudo(cell) = Lpseudo(cell)-pMove;
                vDMove(cell,:) = (abs(pMove)/cosd(pMoveAng))*(vDMove(cell,:)/norm(vDMove(cell,:)));
    ...
    for cell = 1:nCells %Should this be just pseudoVect instead of fiberDir sometimes
...
%                 following(cell) = 0;
%             end
% %             if cellContact(cell,cellTwo) == 1
% %                 if outgrowth(cellTwo) 
% %                     following(cell) = 0;
% %                 end
% %                 if contracting()
% %                 end
...
%         end
        ...
        %Assigns 'Follow' to cells around the first active cell
        for cellTwo = 1:nCells
            if cell == cellTwo
                continue
            end
            if cellContact(cell,cellTwo) == 1
                if bonds(cellTwo) >= Bmin
                    mFollow(cell,cellTwo) = 1;
                else
                    mFollow(cell,cellTwo) = 0;
                end  
            end
        end
        
        %Checks for 'Follow' and if within range assigns 'Follow'
        for repeat = 1:2
            for cellTwo = 1:nCells
                if cell == cellTwo
                    continue
                end
                if nnz(mFollow(cell,:)) > 0
    %                 following(cell) = 1;
                    if cellContact(cell,cellTwo) == 1
                        if bonds(cellTwo) < Bmin
%                         if not(outgrowth(cellTwo)) || not(contracting(cellTwo))
                            mFollow(cellTwo,cell) = 1;
%                             following(cellTwo) = 1;
                        else
                            mFollow(cellTwo,cell) = 0;
                        end
                    end
    %             else
    %                 following(cell) = 0;
...
%             end
            if nnz(mFollow(cell,:)) > 0
                following(cell) = 1;
            elseif nnz(cellContact(cell,:)) == 0
                following(cell) = 0;
            elseif nnz(mFollow(cell,:)) == 0
                following(cell) = 0;
            end
%         end
...
%     dMove = zeros(nCells,1);
    ...
    if pos == runTime/dt*.25
        fprintf('25%%...');
    elseif pos == runTime/dt*.5
        fprintf('50%%...');
    elseif pos == runTime/dt*.75
        fprintf('75%%...');
    elseif pos == runTime/dt
        endTime = toc;
        fprintf('100%%...Elapsed time %d seconds. Now Graphing.\n',round(endTime));
    end
...
% curvey = zeros(nCells,1);
% curve(:) = objectArray(curvey);

for cell = 1:nCells
    curve(cell) = animatedline('Color',abs(rand(3,1)),'LineWidth',2);
end
% curve1 = animatedline('Color','m','LineWidth',2);
% curve2 = animatedline('Color','b','LineWidth',2);
...
% sph = sphere;
% axes_handle = fiberDir(cell,:);
% [x,y,z] = ellipsoid(axes_handle,cellAx(cell,1),cellAx(cell,2),cellAx(cell,3));
% if j == 1
%     rotAx(cell,:) = cross([1 0 0],gFiberDir(j,:,cell));
%     rotAng(cell) = acosd(dot([1 0 0],gFiberDir(j,:,cell))/(norm([1 0 0])*norm(gFiberDir(j,:,cell))));
% else
%     rotAx(cell,:) = cross(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell));
%     rotAng(cell) = acosd(dot(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell))/(norm(gFiberDir(j-1,:,cell))*norm(gFiberDir(j,:,cell))));
% end
% uvGFiberDir(cell,:,2) = gFiberDir(j,:,cell)/norm(gFiberDir(j,:,cell));
% uvGFiberDir(cell,:,1) = gFiberDir(j-1,:,cell)/norm(gFiberDir(j-1,:,cell));
if animation == 1
    for j=1:pos-1 %3D Animation plot
        for cell = 1:nCells
            [x,y,z] = ellipsoid(0,0,0,gCellAx(j,3,cell),gCellAx(j,2,cell),gCellAx(j,1,cell));
%             [x,y,z] = ellipsoid(rw(j,1,cell),rw(j,2,cell),rw(j,3,cell),gCellAx(j,1,cell),gCellAx(j,2,cell),gCellAx(j,3,cell));
%             [x,y,z] = ellipsoid(gFiberDir(j,1,cell),gFiberDir(j,2,cell),gFiberDir(j,3,cell),gCellAx(j,1,cell),gCellAx(j,2,cell),gCellAx(j,3,cell));

            addpoints(curve(cell),rw(j,1,cell),rw(j,2,cell),rw(j,3,cell))
            head(cell) = surf(x+rw(j,1,cell),y+rw(j,2,cell),z+rw(j,3,cell));
%             head(cell) = surf(x+rw(j,1,cell),y,z);
%             gPseudoVect(pos,:,cell)
% %             if norm(gPV(j,:,cell)) == 0
% %                 if j == 1
% %                     rotAx(cell,:) = cross([1 0 0],gFiberDir(j,:,cell));
% %                     uvGFiberDir(cell,:,2) = gFiberDir(j,:,cell)/norm(gFiberDir(j,:,cell));
% %                     rotAng(cell) = acosd(dot([1 0 0],uvGFiberDir(cell,:,2)));
% %     %                 rotAng(cell) = acosd(dot([1 0 0],uvGFiberDir(j,:,cell))/(norm([1 0 0])*norm(gFiberDir(j,:,cell))));
% %                     rotate(head(cell),rotAx(cell,:),rotAng(cell),rw(j,:,cell));
% %                 elseif gFiberDir(j,:,cell) ~= gFiberDir(j-1,:,cell)
% %                     rotAx(cell,:) = cross(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell));
% %                     uvGFiberDir(cell,:,2) = gFiberDir(j,:,cell)/norm(gFiberDir(j,:,cell));
% %                     uvGFiberDir(cell,:,1) = gFiberDir(j-1,:,cell)/norm(gFiberDir(j-1,:,cell));
% %                     rotAng(cell) = acosd(dot(uvGFiberDir(cell,:,1),uvGFiberDir(cell,:,2)));
% %     %                 rotAng(cell) = acosd(dot(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell))/(norm(gFiberDir(j-1,:,cell))*norm(gFiberDir(j,:,cell))));
% %                     rotate(head(cell),rotAx(cell,:),rotAng(cell),rw(j,:,cell));
% %                 end
% %             else
% %                 if j == 1
% %                     rotAx(cell,:) = cross([1 0 0],gPV(j,:,cell));
% %                     uvGFiberDir(cell,:,2) = gPV(j,:,cell)/norm(gPV(j,:,cell));
% %                     rotAng(cell) = acosd(dot([1 0 0],uvGFiberDir(cell,:,2)));
% %     %                 rotAng(cell) = acosd(dot([1 0 0],uvGFiberDir(j,:,cell))/(norm([1 0 0])*norm(gFiberDir(j,:,cell))));
% %                     rotate(head(cell),rotAx(cell,:),rotAng(cell),rw(j,:,cell));
% %                 elseif gPV(j,:,cell) ~= gPV(j-1,:,cell) 
% %                     if norm(gPV(j-1,:,cell)) > 0
% %                         rotAx(cell,:) = cross(gPV(j-1,:,cell),gPV(j,:,cell));
% %                         uvGFiberDir(cell,:,2) = gPV(j,:,cell)/norm(gPV(j,:,cell));
% %                         uvGFiberDir(cell,:,1) = gPV(j-1,:,cell)/norm(gPV(j-1,:,cell));
% %                         rotAng(cell) = acosd(dot(uvGFiberDir(cell,:,1),uvGFiberDir(cell,:,2)));
% %         %                 rotAng(cell) = acosd(dot(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell))/(norm(gFiberDir(j-1,:,cell))*norm(gFiberDir(j,:,cell))));
% %                         rotate(head(cell),rotAx(cell,:),rotAng(cell),rw(j,:,cell));
% %                     end
...
%             rotate(S,[0 0 1],atand(ve(2)/ve(1)))
%             rotate(S,cross([0 0 1],ve),acosd(ve(3)/norm(ve)))
%                 rotate(head(cell),[0 0 1],atand(gVe(j,2,cell)/gVe(j,1,cell)),rw(j,:,cell))
%                 rotate(head(cell),cross([0 0 1],gVe(j,:,cell)),acosd(gVe(j,3,cell)/norm(gVe(j,:,cell))),rw(j,:,cell))
...
%                 uvGFiberDir(cell,:,2) = gFiberDir(j,:,cell)/norm(gFiberDir(j,:,cell));
%                 uvGFiberDir(cell,:,1) = gFiberDir(j-1,:,cell)/norm(gFiberDir(j-1,:,cell));
%                 rotAng(cell) = atan2d(norm(cross(uvGFiberDir(cell,:,1),uvGFiberDir(cell,:,2))),dot(uvGFiberDir(cell,:,1),uvGFiberDir(cell,:,2)));
% %                 rotAng(cell) = acosd(dot(gFiberDir(j-1,:,cell),gFiberDir(j,:,cell))/(norm(gFiberDir(j-1,:,cell))*norm(gFiberDir(j,:,cell))));
%                 rotate(head(cell),rotAx(cell,:),rotAng(cell),rw(j,:,cell));
%             end

            
%             if gFiberDir(j,:,cell) ~= gFiberDir(j-1,:,cell)
%                 rotate(head(cell),rotAx(cell,:),rotAng(cell),randomWalk(j,:,cell));
%             end
%             head(cell) = surf(x*rCell(1)+randomWalk(j,1,cell),y*rCell(1)+randomWalk(j,2,cell),z*rCell(1)+randomWalk(j,3,cell));
            alpha(head(cell),0.25);
            if FNET(j,:,cell) ~= rw(j,:,cell)
%                 vHead(cell) = quiver3(randomWalk(j,1,cell),randomWalk(j,2,cell),randomWalk(j,3,cell),FNET(j,1,cell),FNET(j,2,cell),FNET(j,3,cell),'LineWidth',2);
