...
% contactRange = 0;
angAvoid = 30;%..........Radial angle of avoidance from extending pseudopod Need paper value
Fcomp = 5e-10;%....Compressive force during contact (N)(Need a paper value) {5e-10}
...
otX = zeros(nCells);% Previously cSA; Orientation factor
    ...
    for cell = 1:nCells
        ...
        if not(contracting(cell))
            ...
            while(searchLength(cell) < numElmts(cell) && dl < dt*Vpseudo)%Pseudopod searches each element within dl
                ...
                bonds(cell) = sum(Bsites(cell,(searchLength(cell)-searchEnd):searchLength(cell)),2);  %Bond density for 30 elements
...
%             SNSE = SNSE + transpose(SNSE);
        ...
        for cellTwo = 1:nCells
            ...
            elseif ecks(cell,cellTwo) < 0 
                Fp = Fcomp*otX(cell,cellTwo)*(-1*ecks(cell,cellTwo))^(3/2);
                Fp = Fp*nuCellDist(cell,:,cellTwo);
            else
                Fp = -1*alpha*otX(cell,cellTwo)*(ecks(cell,cellTwo)+...
