...
Fadhv = 100e-10;%...............................Adhesive strength constant (N) 1e-8
...
%         SE(cell) = eay*etaSurf + etaCC*tsurf(cell);%Palsson version of SE
        ...
        else%if contracting(cell)
            SE(cell) = fb(cell);
%         else
%             SE(cell) = 0.075*fb(cell)+fv(cell);
        end
        if SE(cell) < 0
...
if animation == 1
    for j=1:pos %3D Animation plot
        for cell = 1:nCells
%             addpoints(curve1,randomWalk(j,1,1),randomWalk(j,2,1),randomWalk(j,3,1))
%             addpoints(curve2,randomWalk(j,1,2),randomWalk(j,2,2),randomWalk(j,3,2))
