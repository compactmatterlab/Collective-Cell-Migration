...
% Model Definitions
runTime = 1*24*3600;%..........................Total runtime for model (sec) 12*4800
...
fprintf('Running %d cell/s over %d day/s. Start Time:  %s \n',nCells,runTime/(24*3600),datestr(now))

%Stat Analysis Toggles
...
% contactRange = 0;
angAvoid = 60;%..........Radial angle of avoidance from extending pseudopod Need paper value
Fcomp = 5e-10;%....Compressive force during contact (N)(Need a paper value) {1e-11}
...
dL = (RGD*0.7*0.9)/((Ltropo+0.067)*Dtropo);%RGD ligand density [lingand/um^2]
...
%New set of vars from newer code
fv = zeros(nCells,1);
F0 = 0;
sl = 0;
ve = zeros(nCells,3);
...
%Generates cell's initial position/polarity (+/- 15um from origin)
for cell=1:nCells
...
%=========================================================================%
    for cell = 1:nCells
        ...
        x = abs(cellPos(pos,1,cell)+pseudoVect(cell,1));
        ...
        dL = (RGD*0.7*0.9)/((Ltropo+0.067)*Dtropo);%RGD ligand density
        ...
        if newAngle(cell)
            ...
            L = zeros(localFibers(cell),1);%.........Distance to intersection
            cs = zeros;
            sn = zeros;
            ...
            for j=1:localFibers(cell)
                ...
                %Calculates new fiber direction
                    ...
                    fiberAngle(cell) = (180*(randi(2)-1))+normrnd(otAngle,sigma);%Aligned fibers
                    v1 = (L(j)*cosd(fiberAngle(cell))*vRef)/norm(vRef);%'x' component of new vector
            ...
            dmin(:) = rCell(1)*0.14*(-1);%................Min distance from 
            ...........cell center to non-deformed edge of neighboring cell

            %Calculate cell sphericity
            SAcell(cell) = saellipsoid(cellAx);%................Cell surface area 
            ...
            for j=1:localFibers(cell)
                angle(j,1) = acosd(dot(fiber(j,:),ve)/(norm(fiber(j,:))*norm(ve)));
            end
            ...
            Bpseudo(cell) = Bpseudo(cell)+...
                sum(Bsites(cell,1:searchLength(cell)));
        ...
        elseif bonds(cell) >= Bmin && bonds(cell) < Bmax
            ...
            if searchLength(cell) >= numElmts(cell)
                if rand < 0.5
                    ...
                    Bpseudo(cell) = Bpseudo(cell)+sum(Bsites(cell,1:...
                        searchLength(cell)));
                end
            end
        ...
        Ltouching = mean(cellAx)/4;%Estimated length of fibers touching cell
            ...
            for cellTwo = 1:nCells
                ...
                elseif cellContact(cellOne,cellTwo)
                    surf(cellOne,cellTwo) = 0.25*exp(-5*...
                        (ecks(cellOne,cellTwo)-a(cellOne))^2)*...
...
%                     tsurf(cell) = 0;
%                 else
%                     tsurf(cell) = sum(surf(cell,:),1)/SAcell(cell);
%                 end
...
%         end
        ...
        for cellTwo = 1:nCells
            if not(cellContact(cell,cellTwo)) || cell == cellTwo
                ...
                SN(cell,cellTwo) = surf(cell,cellTwo)*etaCC;
            end
        end
...
%             SNSE = SNSE + transpose(SNSE);
        ...
        for cellTwo = 1:nCells
            ...
            if isnan(Fp(1,1)) == 1 || isinf(Fp(1,1)) == 1
    ...
    %=====================================================================%
    if isempty(contracting) && isempty(cellContact)%....No rxns/No movement 
...
%                     Vz(x) = 0;
        ...
        for cell = 1:nCells
            ...
            dMove(cell) = min([Lpseudo(cell),dMove(cell)]);%.........Eliminates overshoot
            if contracting(cell)
    ...
    %Reset Variables for next complete cell loop
    Vx = zeros(nCells,1);
    Vy = zeros(nCells,1);
    Vz = zeros(nCells,1);
    FNet = zeros(nCells,3);
...
for i=1:nCells % 3D scatter plot
    scatter3(randomWalk(1,1,i),randomWalk(1,2,i),randomWalk(1,3,i),'filled','MarkerEdgeColor','g','MarkerFaceColor','g','LineWidth',2)
...
%     plot3(cont_pos(:,1,i),cont_pos(:,2,i),cont_pos(:,3,i),'Color','r')
    scatter3(randomWalk(end,1,i),randomWalk(end,2,i),randomWalk(end,3,i),'MarkerEdgeColor','r','MarkerFaceColor','r')
end 
