...
RGD = 5;%................................RGD Peptides/monomer/tropocollagen
...
%             contracting(cell) = 0;
    ...
    for cell = 1:nCells
        ...
        %contracting
        for cellOne = 1:nCells
            if bonds(cellOne) > Bmax % TEMP FOR ADHES TEST; CHANGE TO BMAX 
                for cellTwo = 1:nCells
...
%                 kT_eff = 0;
%             else
%                 kT_eff = (F0*kecm(cell)^2*Lps2(cell)^3)/(F0+kecm(cell)*Lps2(cell))^2;
%             end
%         else
            if Lpseudo(cell) == 0
                kT_eff = 0;
...
%         end
        ...
        for cellOne = 1:nCells 
            for cellTwo = 1:nCells
                ...
                di(cellOne,cellTwo) = sqrt((b(cellOne)*cosd(pseudoCellAng(cellOne,cellTwo)))^2 ...
...
%                     (F0+kecm(cell)*Lpseudo(cell));
                F(cell) = (F0*kecm(cell)*Lps2(cell))/...
                    (F0+kecm(cell)*Lps2(cell));%Dunno about this; Trying to preserve constant pulling force
            end
...
%         FNET(pos,:,cell) = (FNet(cell,:).*1e9)+cellPos(pos,:,cell);%.............For graphing purposes
    end
    %=====================================================================%
    % END cell loop but still in deltaT
    %=====================================================================%
%     if nnz(cellContact) > 0
%         if nnz(SNSE) < nCells^2
%             fprintf('SNSE is zero somewhere')
%         end
%     end
...
%             cVel(u,:) = [Vx(u),Vy(u),Vz(u)];%..........Cell velocity vector
        ...
        for cell = 1:nCells
            ...
            if bonds(cell) > Bmin
                if Lpseudo(cell) > 0 && norm(vDMove(cell,:)) > 0
    %                 Lpseudo(cell) = Lpseudo(cell)-dot(fiberDir(cell,:),vDMove(cell,:))/norm(fiberDir(cell,:)); 
                    Lpseudo(cell) = Lpseudo(cell)-dot(pseudoVect(cell,:),vDMove(cell,:))/norm(pseudoVect(cell,:)); 
                end
            end
            %Stores new cell position and direction
...
%                     [row,col] = find(cellContact(cell,:) == 1);
%                     dMMax(col) = dMove(cell);
% %                     if nnz(col) > 1
% %                         fprintf('dMax test')
% %                     end
%                 end
...
%            if ~contracting(cell)
%                if nnz(cellContact(cell,:)) > 0
%                    dMove(cell) = min(dMMax(cell),dMove(cell));
%                end
%            end
%         end
%         for cell = 1:nCells
...
%             end
%             %Stores new cell position and direction
%             cellPos(pos+1,:,cell) = dMove(cell)*nuVel(cell,:)+...
%                 cellPos(pos,:,cell);
%         end
    end 
...
%             [x,y,z] = ellipsoid(gFiberDir(j,1,cell),gFiberDir(j,2,cell),gFiberDir(j,3,cell),gCellAx(j,1,cell),gCellAx(j,2,cell),gCellAx(j,3,cell));
            ...
            addpoints(curve(cell),randomWalk(j,1,cell),randomWalk(j,2,cell),randomWalk(j,3,cell))
            head(cell) = surf(x+randomWalk(j,1,cell),y+randomWalk(j,2,cell),z+randomWalk(j,3,cell));
...
%             head(cell) = surf(x*rCell(1)+randomWalk(j,1,cell),y*rCell(1)+randomWalk(j,2,cell),z*rCell(1)+randomWalk(j,3,cell));
            ...
            if FNET(j,:,cell) ~= randomWalk(j,:,cell)
%                 vHead(cell) = quiver3(randomWalk(j,1,cell),randomWalk(j,2,cell),randomWalk(j,3,cell),FNET(j,1,cell),FNET(j,2,cell),FNET(j,3,cell),'LineWidth',2);
                vHead(cell) = quiver3(randomWalk(j,1,cell),randomWalk(j,2,cell),randomWalk(j,3,cell),FNET(j,1,cell),FNET(j,2,cell),FNET(j,3,cell),'LineWidth',3);
            end
        end
        drawnow
        delete(vHead(:));
